...
class UserModule extends BaseModule
{
    ...
    public static function formatAlbumFullUrl($jsonStr)
    {
        ...
        foreach ($arr as $item) { // $item 即为object
            $material = Db::name('oss_material')->where('object', $item)->find();
            if (! $material) {
                continue;
            }
            $aRet[] = [
                'object' => $material['object'],
                'full_url' => AliyunOssModule::getFullUrl($item)
            ];
        }
    ...
    public static function formatVideoFullUrl($jsonStr, $userId = null)
    {
        ...
        foreach ($arr as $item) { // $item 即为object
            ...
            $tmp['full_url'] = AliyunOssModule::getFullUrl($material['object']);
            $tmp['cover_img'] = AliyunOssModule::getVideoCoverimg($tmp['full_url']);
