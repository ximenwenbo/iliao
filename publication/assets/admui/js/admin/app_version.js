  ...
  $(function() {
    ...
    oTable = $('.dataTable').DataTable(
      $.concatCpt('dataTable', {
        ...
        ajax: function(data, callback) {
          $.ajax({
            url: '/admin/app_version/ListAjax',
            cache: false,
            dataType: 'JSON',
            data: $.extend(
    ...
    let format = function(d){
        return '<table class="table to_table table-bordered text-nowrap pl-50 mb-0" cellpadding="4">' +
                    '<tr>' +
                        '<td width="25%">保存路径：' + d.old_url +'&nbsp;&nbsp;&nbsp;<a href=" '+ d.sdk_url +'" target="_blank">下载</a>' +'</td>' +
                        '<td width="25%">更新说明：'+ d.update_msg + '</td>'+
                    '</tr>' +
                '</table>';
    };
    // 展开关闭详情时的事件监听
