$(function() {
    ...
    $('#uploadImg').fileupload({
        ...
        success:function (data) {
            ...
            if(dataStr.code === 1 && dataStr.data.type ===1){
                $("#gift_thumb_dir").val(dataStr.data.save_dir);
            }
        }
    })
    //图片添加完成后触发的事件
    ...
    function validate(file) {
        ...
        //验证图片格式
        if (!/.(gif|jpg|jpeg|png|gif|jpg|png|svga)$/.test(fileName)) {
            console.log("文件格式不正确");
            return true;
        }
        //验证excell表格式
...
$(function() {
    ...
    $('#uploadImg_1').fileupload({
        ...
        maxNumberOfFiles : 2,//最大上传文件数目
        maxFileSize : 20000000000, // 文件上限1MB
        minFileSize : 100,//文件下限  100b
        messages : {
            acceptFileTypes : '文件类型不匹配',
            maxFileSize : '文件过大',
        ...
        success:function (data) {
            ...
            console.log(dataStr);
            if(dataStr.code === 1 && dataStr.data.type ===1){
                $("#gift_gif_dir").val(dataStr.data.save_dir);
            }
        }
    })
    //图片添加完成后触发的事件
