    ...
    $(function() {
        ...
        oTable = $('.dataTable').DataTable(
            ...
            ajax: function(data, callback) {
                $.ajax({
                url: '/admin/merchant_customer/index',
                    cache: false,
                    dataType: 'JSON',
                    data: $.extend(
        ...
        let format = function(d){
            return '<table class="table to_table table-bordered text-nowrap pl-50 mb-0" cellpadding="5">' +
                        '<tr>' +
                            '<td width="25%">支付渠道：'+ d.pay_channel + '</td>' +
                            '<td width="25%">充值金币：'+ d.coin + '</td>' +
                            '<td></td>' +
                            '<td></td>' +
                        '</tr>' +
                        '<tr>' +
                        '<td>完成时间：'+ d.finish_time + '</td>' +
                        '<td>更新时间：'+ d.update_time + '</td>' +
                        '<td width="25%">唯一标识：'+ d.extra_id + '</td>' +
                        '<td width="25%">备注说明：'+ d.error_msg + '</td>' +
                        '</tr>' +
                    '</table>';
        };
        // 展开关闭详情时的事件监听
        ...
        tb.on('click','#DelOne',function () {
            ...
            layer.confirm('确定删除该信息？', {
                ...
                $.ajax({
                    type: 'POST',
                    url: "/admin/merchant_customer/DelInfo",
                    data: {id:id},
                    dataType: "json",
