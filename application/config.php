...
if (file_exists(CMF_ROOT . "data/conf/config.php")) {
    $config_dir = "data/conf/config.php";
    $runtimeConfig = include CMF_ROOT . $config_dir;
} else {
    $runtimeConfig = [];
}
$configs = [
    ...
    // 视图输出字符串内容替换
    'view_replace_str'        => [
        '__ABS__' => think\Request::instance()->domain(),
    ],
    // 默认跳转页面对应的模板文件
    'dispatch_success_tmpl'   => ROOT_PATH . 'publication/themes/backed/tpl' . DS . 'success_jump.tpl',
    'dispatch_error_tmpl'     => ROOT_PATH . 'publication/themes/backed/tpl' . DS . 'error_jump.tpl',
    // +----------------------------------------------------------------------
    // | 异常及错误设置
    // +----------------------------------------------------------------------
    // 异常页面的模板文件
    ...
    'cmf_default_theme'       => 'fronted',
    'cmf_admin_theme_path'    => 'themes/',
    'cmf_admin_default_theme' => 'backed',
];
