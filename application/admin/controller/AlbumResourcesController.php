...
class AlbumResourcesController extends AdminBaseController
{
    /**
     * 相册列表
     * @author zjy
    ...
    public function ListAjax()
    {
        ...
        if(!empty($result['data'])){
            foreach ($result['data'] as $val)
            {
                ...
                {
                    case 1:
                        $resource = '<div class="gallery-wrapper">
                                        <a class="galpop-single" href="javascript:void(0)" onclick="NewGalpop('."'".$oss_url."'".')">
                                            <img src="'.$oss_url.'" class="img-thumbnail" alt="" width="50px" height="50px"/>
                                        </a>
                                    </div>';
                        break;
                    case 2:
                        $resource = '<i class="icon wb-play" aria-hidden="true">&nbsp;</i><a href="javascript:void(0)" onclick="userVideo('."'{$val['id']}'".')">点击查看</a>';
                        break;
                    default:
                        $resource = '';
                        break;
                ...
                $filed =  [
                    ...
                    'class_id' => $val['class_id'] == 1 ? '个人相册' : '个人视频',
                    'object' => $resource,
    ...
    public function WatchVideo(){
        $id = $this->request->param('obj');
        try{
            $oss = Db::name('oss_material')->where(['id'=>$id])->find();
            $user_info = Db::name('user')->field('user_nickname,mobile')->where(['id'=>$oss['user_id']])->find();
            $oss_url = MaterialService::getFullUrl($oss['object']);
            $this->assign('url',$oss_url);
    ...
    public function toExamine()
    {
        ...
        if(Request::instance()->isPost()){
            ...
            $result = ResourcesService::UpdateB(['id'=>$condition['id']],$condition);
            if(empty($result))
            {
            ...
            else
            {
                ...
                if ($params['status'] == 2) { // 拒绝
                    YuntongxinService::pushSysNotice($authRow['user_id'], 'SYS_ALBUM_AUTH_SUCCESS');
                }else if($params['status'] == 10) { // 拒绝
                    YuntongxinService::pushSysNotice($authRow['user_id'], 'SYS_ALBUM_AUTH_FAIL');
                }
