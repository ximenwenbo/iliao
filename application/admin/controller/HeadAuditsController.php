...
use app\admin\service\HeadAuditsService;
...
class HeadAuditsController extends AdminBaseController
{
    /**
     * 头像审核列表
     * @author zjy
    ...
    public function ListAjax()
    {
        ...
        $result = HeadAuditsService::RList($condition);
        //var_dump($result);die;
        //调用列表方法
        $data = [];
        if(!empty($result['data'])){
            foreach ($result['data'] as $val)
            {
                ...
                $filed =  [
                    ...
                    'class_id' => $val['class_id'] == 6 ? '用户头像' : '类型错误',
                    'object' => $resource,
                    'status' => HeadAuditsService::statusListSelect($val['status']),
    ...
    public function toExamine()
    {
        ...
        if(Request::instance()->isPost()){
            ...
            $result = HeadAuditsService::UpdateB(['id'=>$condition['id']],$condition);
            if(empty($result))
            {
                ...
                if ($params['status'] == 2) { // 通过
                    YuntongxinService::pushSysNotice($authRow['user_id'], 'SYS_HEAD_AUDITS_SUCCESS');
                } else if($params['status'] == 3){ // 拒绝
                    YuntongxinService::pushSysNotice($authRow['user_id'], 'SYS_HEAD_AUDITS_FAIL');
                }
        ...
        $info = HeadAuditsService::ToInfo($id);
        if(empty($info)){
