...
class VideoResourcesController extends AdminBaseController
{
    /**
     * 视频列表
     * @author zjy
    ...
    public function ListAjax()
    {
        ...
        if(!empty($result['data'])){
            foreach ($result['data'] as $val)
            {
                ...
                {
                    case 1:
                        $resource = '<div onclick="ZoomDisplay('."'正面照'".",'{$oss_url}'".')"><img src="'.$oss_url.'" alt="正面照" title="正面照" width="50" height="50"></div>';
                        break;
                    case 2:
        ...
        if(Request::instance()->isPost()){
            ...
            else
            {
                ...
                if ($params['status'] == 2) { // 拒绝
                    YuntongxinService::pushSysNotice($authRow['user_id'], 'SYS_VIDEO_AUTH_SUCCESS');
                }else if($params['status'] == 10) { // 拒绝
                    YuntongxinService::pushSysNotice($authRow['user_id'], 'SYS_VIDEO_AUTH_FAIL');
                }
