...
use app\admin\model\LiveManagerModel;
use think\Exception;
use think\Log;

class LiveManagerService extends BaseService
{
    /**
     * 机器人列表
    ...
    public static function RList($filter)
    {
        ...
        if(!empty($filter['start_time']) && !empty($filter['end_time']))
        {
            if(strtotime($filter['start_time']) && strtotime($filter['end_time'])){
                ...
                $where .= " and a.create_time between '{$filter['start_time']}' and '{$end_time}'";
            }
