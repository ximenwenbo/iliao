...
use app\admin\model\AgentPromotersModel;
use think\Exception;
use think\Log;

class AgentPromotersService extends BaseService
{
    ...
    public static function RList($filter)
    {
        ...
        {
            $where .= " and (id = {$filter['keywords']} or user_id = {$filter['keywords']})";
        }

        if(!empty($filter['agent_id'])){
            $where .= " and agent_id = {$filter['agent_id']}";
        }
        //搜索开始和结束时间
