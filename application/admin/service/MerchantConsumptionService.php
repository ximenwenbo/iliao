...
class MerchantConsumptionService extends BaseService
{
    ...
    {
        //条件处理
        $where = "a.class_id=3 and a.change_type = 2 and user_id in($user_id)";
        //关键词
        if (!empty($filter['keywords']) && empty($filter['keywords']))
        {
            $where .= " and (a.user_id = {$filter['keywords']} or u.mobile = {$filter['keywords']})  ";
        }
