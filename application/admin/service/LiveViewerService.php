...
use app\admin\model\LiveViewerModel;
use think\Exception;
use think\Log;

class LiveViewerService extends BaseService
{
    /**
     * 机器人列表
    ...
    public static function RList($filter)
    {
        ...
        {
            $where .= " and (a.live_id = {$filter['live_id']})  ";
        }
    ...
    public static function statusList($key = -1){
        $aRet = [
            0 => '<span style="color:#c6d3d7;">离开房间</span>',
            1 => '<span style="color:#3e8ef7;">在房间</span>',
        ];
        if($key >= 0){
