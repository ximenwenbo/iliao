...
use app\admin\model\LiveMikeModel;
use think\Exception;
use think\Log;

class LiveMikeService extends BaseService
{
    /**
     * 机器人列表
    ...
    public static function RList($filter)
    {
        ...
        if (!empty($filter['keywords']))
        {
            if(is_numeric($filter['keywords'])){
                $where .= " and (a.launch_uid = {$filter['keywords']} or a.accept_uid = {$filter['keywords']} ) ";
            }else{
     ...
     * @return array|string
     */
    public static function resultList($key = -1){
        $aRet = [
            0 => '平局',
            1 => '发起者胜',
            2 => '接受者胜',
