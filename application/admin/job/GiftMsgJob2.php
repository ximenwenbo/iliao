...
class GiftMsgJob2
{
    ...
    public function fire(Job $job, $data)
    {
        try {
            ...
            $imgUrl = $this->getLocalImgUrl($giftFind['icon_img']);
            ...
            do {
                ...
                //拼装消息体
                $content = [
                    'code' => 'live_gift_notice',
                    'subject' => '直播礼物弹幕消息',
                    'content' => [
                        'style' => 2, // 2:直播件礼物消息，全平台滚屏推送效果
                        'content' => $aData
                    ]
                ];
                ...
                if ($result == null || $result['ActionStatus'] != 'OK') {
                    // 签名生成失败
                    Log::write(sprintf('%s：[任务执行异常]直播礼物弹幕消息发送失败：%s', __METHOD__, var_export($result, true)),'error');
                }
            ...
            Log::write(sprintf('%s：系统错误，直播礼物弹幕消息发送[任务执行异常]：%s IN: %s LINE: %s', __METHOD__, $e->getMessage(), $e->getFile(), $e->getLine()),'error');
        }
    ...
    private function getReceiveUser($type, $page, $pageSize)
    {
        ...
        if ($type == 'all_viewer') {
            $userIds = Db::name('live_home_viewer')
                ->alias('v')
                ->join('user u', 'u.id = v.user_id AND u.user_type = 2')
                ->where('v.status', 1)
                ->page($page, $pageSize)
                ->column('v.user_id');
        }
